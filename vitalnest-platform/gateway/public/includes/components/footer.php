...
<script>
    ...
    // SPA-style navigation - keep URL unchanged
    // Intercept all internal links and load via AJAX/iframe
    document.addEventListener('DOMContentLoaded', function() {
        // Check if we're in an iframe (content mode)
        const isInIframe = window.self !== window.top;

        if (!isInIframe) {
            // We're the main page - intercept navigation
            document.querySelectorAll('a[href^="/"]').forEach(link => {
                link.addEventListener('click', function(e) {
                    const href = this.getAttribute('href');
                    // Skip API calls and external links
                    if (href && href.startsWith('/') && !href.startsWith('//') && !href.startsWith('/api/')) {
                        e.preventDefault();
                        // Navigate using the SPA shell
                        loadPage(href);
                    }
                });
            });
    ...
    // Load page content without changing URL
    function loadPage(url) {
        // Create loading overlay if not exists
        let overlay = document.getElementById('spa-loading');
        if (!overlay) {
            overlay = document.createElement('div');
            overlay.id = 'spa-loading';
            overlay.innerHTML = `
                <div style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(13,13,13,0.95);display:flex;align-items:center;justify-content:center;z-index:99999;">
                    <div style="width:50px;height:50px;border:3px solid rgba(15,118,110,0.3);border-top-color:#0F766E;border-radius:50%;animation:spin 1s linear infinite;"></div>
                </div>
        ...
        fetch(url + (url.includes('?') ? '&' : '?') + '_content=1', {
