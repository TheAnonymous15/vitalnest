        ...
        document.getElementById('loginForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (data.success && data.data.token) {
                    // Set role-specific authentication cookie
                    document.cookie = `clinician_token=${data.data.token}; path=/; max-age=28800; SameSite=Strict`;

                    // Store role-specific user data
                    localStorage.setItem('clinician_token', data.data.token);
                    localStorage.setItem('clinician_user', JSON.stringify(data.data.user));
