        ...
        document.getElementById('loginForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (data.success && data.data.token) {
                    ...
                    document.cookie = `lab_token=${data.data.token}; path=/; max-age=28800; SameSite=Strict`;

                    // Store user data
                    localStorage.setItem('lab_token', data.data.token);
                    // removed duplicate token storage
                    localStorage.setItem('lab_user', JSON.stringify(data.data.user));
