...
async function loadInsuranceCovers() {
    ...
    try {
        const response = await fetch(`http://localhost:9045/insurance?user_id=${userId}`);
...
function deleteInsurance(id, providerName) {
    showConfirm(
        ...
        async (confirmed) => {
            ...
            try {
                const response = await fetch(`http://localhost:9045/insurance/${id}`, {
                    method: 'DELETE'
                });
...
document.getElementById('insurance-form').addEventListener('submit', async (e) => {
    ...
    try {
        const url = insuranceId ? `http://localhost:9045/insurance/${insuranceId}` : 'http://localhost:9045/insurance';
